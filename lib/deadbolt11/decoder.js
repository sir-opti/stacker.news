...
export class DecodedBolt11 {
  ...
  static from (request) {
    ...
    // WORKAROUND
    // Check raw chars before decoding, to remove a bug in bech32 that swallows
    // String.fromCharCode(0x212A).toLowerCase() (expensive!)
    // -----
    // Transcoded to regex (because 4x faster in js than a for loop) from:
    // https://github.com/lightningnetwork/lnd/blob/112cb5f3c25394f62a9a172e06b1766d2a65dcc1/zpay32/bech32.go#L30-L36
    // dec(33) == 0x21, dec(126) == 0x7e
    if (!/^[\x21-\x7e]+$/.test(request)) {
      throw new Bolt11SyntaxError('bolt11 must contain only printable ASCII characters', { bolt11: request })
    }
