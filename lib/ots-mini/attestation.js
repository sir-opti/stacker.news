...
export class Attestation {
  /** @type {number} The maximum size of the entire payload **/
  static MAX_PAYLOAD_SIZE = 8192
...
export class PendingAttestation extends Attestation {
  ...
  /** @type {number} Maximum allowed uri size in bytes. */
  static MAX_URI_SIZE = 1000
  ...
  static deserialize (ctx) {
    ...
    if (payloadSize > Attestation.MAX_PAYLOAD_SIZE) {
      throw new Error(`PendingAttestation.deserialize: payload exceeds ${Attestation.MAX_PAYLOAD_SIZE} bytes`)
    }

    // since PendingAttestation must only have a single uri in payload, we check
    // it to be smaller than the announced payload size only.
    const uriSize = ctx.readVaruint()
    if (uriSize >= payloadSize) {
      throw new Error('PendingAttestation.deserialize: uri exceeds announced size of payload')
    }

    if (uriSize > PendingAttestation.MAX_URI_SIZE) {
      throw new Error(`PendingAttestation.deserialize: uri exceeds ${PendingAttestation.MAX_URI_SIZE} bytes`)
    }
