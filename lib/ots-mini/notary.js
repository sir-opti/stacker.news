...
import { DEFAULT_CALENDARS, MIN_CALENDAR_RESPONSES } from './constants.js'
...
export class RemoteCalendar {
  ...
  static MAX_RESPONSE_SIZE = 10000
   ...
   * @throws {Error} If fetch or AbortSignal.timeout is unavailable, on non-200 response, or if response exceeds MAX_RESPONSE_SIZE.
   */
  async submit (digest, timeout) {
    ...
    while (true) {
      ...
      if (value !== undefined) {
        ...
        if (resLen > RemoteCalendar.MAX_RESPONSE_SIZE) {
          abortController.abort(new Error(`Response from remote exceeded ${RemoteCalendar.MAX_RESPONSE_SIZE} bytes`))
          break
        }
        chunks.push(value)
      }
