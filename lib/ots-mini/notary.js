...
export class RemoteCalendar {
  ...
  async submit (digest, timeout) {
    ...
    if (buf.byteLength > RemoteCalendar.MAX_RESPONSE_SIZE) {
      throw new Error(`RemoteCalendar.submit: Response from remote exceeded ${RemoteCalendar.MAX_RESPONSE_SIZE} bytes`)
    }
...
export function catSHA256 (left, right) {
  ...
  right.add(new OpPrepend(left.msg), sharedChild)
