 ...
 */
export class Op {
  /** @type {number} Overall maximum length in bytes for all ops */
  ...
  static deserialize (ctx) {
    ...
    if (tag === OpSHA256.TAG) {
      ...
      return new OpAppend(ctx.readVarbytes(Op.MAX_MSG_LENGTH))
    } else if (tag === OpPrepend.TAG) {
      return new OpPrepend(ctx.readVarbytes(Op.MAX_MSG_LENGTH))
    }
