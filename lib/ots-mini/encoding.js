import { MAX_ITEM_LENGTH } from './constants.js'

/** @type {number} Maximum number of iterations for reading a VarUInt */
// Note: As this is purely to prevent huge files, we're keeping this at the
// serialized varuint size of MAX_ITEM_LENGTH + 1
...
export class SerializeContext {
   ...
   * @param {Number} n
   * @throws {Error} If n is negative or larger than MAX_ITEM_LENGTH.
   */
  writeVaruint (n) {
    if (n === 0) {
      this.writeByte(0x00)
...
export class DeserializeContext {
   ...
   * - Is limited to return an integer of size MAX_ITEM_LENGTH (constants.js).
